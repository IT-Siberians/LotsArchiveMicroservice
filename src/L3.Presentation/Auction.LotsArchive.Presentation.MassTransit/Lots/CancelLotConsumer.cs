...
public class CancelLotConsumer(
    IValidator<AddWithdrawnLotCommand> validator,
    ICommandHandler<AddWithdrawnLotCommand> handler,
    ILogger<CancelLotConsumer> logger)
        : IConsumer<CancelLotEvent>
{
    public async Task Consume(ConsumeContext<CancelLotEvent> context)
    {
        var command = new AddWithdrawnLotCommand(
            new LotModel(
